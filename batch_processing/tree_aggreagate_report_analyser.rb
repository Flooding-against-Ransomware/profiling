...
def aggregate_json( target_dir, output_dir )
  files = Pathname.new( target_dir ).children.select { |f| f.extname == '.json' }
  ...
  save_results( target_dir, output_dir, grouped_data )
...
if ARGV.length != 2
  puts "Usage: ruby tree_aggregate_report_analyser.rb <target_directory> <output_directory>"
  exit(1)
end
...
aggregate_json( target_dir, output_dir )